# -*- coding: utf-8 -*-
"""
Created on Tue Mar 31 15:20:03 2020
...
def predict(final,carrera):
    ...
    dataTest1 = dataTest[["rateReprobadas","rateAnuladas","rateAprobadas","mediaAPRP","mediaAP","segMatActual","terMatActual","terMatTot"]]#"numMatSem",
    ...
    """
    param_grid = {    
            'max_depth': range(6,7,3),
            'max_features': ['auto'],
            'min_samples_leaf': range(7,8,3),
    ...
    df = pd.DataFrame(y_pred['dropout'])
    dataF = dataF.reset_index(drop=True)
    dataF.drop("dropout",1,inplace=True)
    df = pd.concat([dataF, df], axis=1)
